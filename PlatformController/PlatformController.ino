...
{
  if (!CALIBRATED()) // Begin calibration procedure
  {
    ...
    ACTUATE_ELEVATOR();
    if (IMU.getAccelZ_mss() > tempZMeas)
    {
      tempZMeas = IMU.getAccelZ_mss();
    }

    if (totalInterrupts0 >= 12)
    {
      Serial.println("Complete!");
      EP = 0;
      STOP_ELEVATOR();
      IMU.setAccelCalZ(tempZMeas, 1);
      Serial.println(tempZMeas);
      calibrated = true;
    }
    //    switch (calibrationStage)
    //    {
    //      case 0: // Find direction of horizon
    //        IMU.readSensor();
    //        tempZMeas = IMU.getAccelZ_mss();
    //        EP = 25;
    //        Serial.println("Attempting to move elevator");
    //        ACTUATE_ELEVATOR(); // Move elevator by a small amount
    //        delay(200);
    //        if (tempZMeas - IMU.getAccelZ_mss() > 0.2f) // If this is true, we now know that we are currently moving away from the horizon
    //        {
    //          Serial.println("Reversing...");
    //          EP = -25;
    //          ACTUATE_ELEVATOR(); // Move elevator back to original position
    //          delay(200);
    //          STOP_ELEVATOR();
    //          directionTowardsLevel = -1;
    //          calibrationStage = 1;
    //        }
    //        else if (tempZMeas - IMU.getAccelZ_mss() < -0.2f) // If this is true, we now know that we are currently moving towards the horizon
    //        {
    ...
    //          calibrationStage = 1;
    //        }
    //        else // This means that the motor has stalled... in this case, move in the opposite direction for a longer time
    //        {
    //          Serial.println("Stall detected, reversing!");
    //          EP = -25;
    //          ACTUATE_ELEVATOR(); // Move elevator back to original position
    //          delay(400);
    //          Serial.println("Stopped...");
    ...
    //        }
    //        break;
    //      case 1: // Find horizon and set position at that place
    //        Serial.println("Trying to find horizon..." + String(EP));
    //        EP = directionTowardsLevel * 25;
    //        ACTUATE_ELEVATOR();
    //        if (tempZMeas  < IMU.getAccelZ_mss())
    //        {
    //          tempZMeas =  IMU.getAccelZ_mss();
    //        }
    //        else if (tempZMeas -  IMU.getAccelZ_mss() > 0.2f)
    //        {
    //          STOP_ELEVATOR();
    //          calibrationStage = 2;
    //        }
