...
float K_i = 0.0f; // For integral term
...
void HTTPCoroutine( void* pvParameters)
{
  for (;;) {
    ...
    delay(25); // Prevent the main core 1 from overloading
  }
}
...
void DETERMINE_MOVEMENT_REQ()
{
  ...
  if (b_Roll > 0)
  {
    EP = - Kp_e * (p_e + (pitch_rate_error * K_i)); // More aggressive controller configuration because of the extra torque needed to elevate the phone
  }
  else
  {
    EP = - Kp_d * (p_e + (pitch_rate_error * K_i)); // Less aggressive controller for depression of mechanism, since the torque of the phone is biased in that direction
  }
  // Anti-windup
